"""Discount Factor 적용 및 CUSUM 기반 Anomaly Detection (GECCO Dataset)
...
# Sensor names (9 sensors, Label excluded)
SENSOR_NAMES = ['Tp', 'Cl', 'pH', 'Redox', 'Leit', 'Trueb', 'Cl_2', 'Fm', 'Fm_2']
NUM_CHANNELS = len(SENSOR_NAMES)

# GECCO CSV path (for ground truth labels)
GECCO_CSV_PATH = './dataset/TSB-AD-M/173_GECCO_id_1_Sensor_tr_16165_1st_16265.csv'
...
def find_result_path(seq_len, pred_len):
    """Find result folder matching GECCO."""
    results_dir = './results/'
    if not os.path.exists(results_dir):
        print(f"Error: results folder not found: {results_dir}")
        return None
    candidates = [f for f in os.listdir(results_dir)
                  if 'GECCO' in f and f'seq{seq_len}' in f and f'pred{pred_len}' in f]
    if len(candidates) == 0:
        print(f"Error: Cannot find GECCO result folder with seq_len={seq_len}, pred_len={pred_len}")
        return None
...
def load_ground_truth_labels():
    """Load ground truth anomaly labels from GECCO CSV"""
    try:
        df = pd.read_csv(GECCO_CSV_PATH)
...
def plot_all_channels(all_results, gt_labels_test=None):
    ...
    fig.suptitle(f'GECCO CUSUM per Sensor (Discount={DISCOUNT_FACTOR})',
    ...
    save_dir = './figures_GECCO'
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, f'GECCO_CUSUM_AllChannels_DF{DISCOUNT_FACTOR}_b0.5.png')
...
def main():
    print("=" * 60)
    print("GECCO Anomaly Detection - Per-Channel Discount CUSUM")
    print("=" * 60)
    print(f"  SEQ_LEN={SEQ_LEN}, PRED_LEN={PRED_LEN}, DISCOUNT={DISCOUNT_FACTOR}")
    print(f"  Channels: {NUM_CHANNELS} ({', '.join(SENSOR_NAMES)})")
