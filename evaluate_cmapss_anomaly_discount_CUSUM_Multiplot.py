...
CHANNEL = 0           # Sensor index to analyze (0-13)
ENGINE_INDEX = 0       # Test engine index (0-19 → engines 81-100)
...
def cusum(error, mu=0, k=0, mode='up'):
    ...
    for t in range(1, len(error)):
        if np.isnan(error[t]):
            ...
            continue
        if mode == 'up':
            S[t] = max(0, S[t-1] + (error[t] - mu - k))
        else:
            S[t] = min(0, S[t-1] + (error[t] - mu + k))
    return S
...
def plot_analysis(result, mse, mae, signed_error, gradient_mse, gradient_mae, smoothed_mse, smoothed_mae,
    ...
    # ===== Plot 1: Prediction Count (how many predictions overlap) =====
    ax1 = axes[0]
    ax1.fill_between(time_steps, pred_count, alpha=0.5, color='orange')
    ax1.axhline(y=pred_len, color='red', linestyle='--', label=f'Max overlap = pred_len ({pred_len})')
    ...
    ax1.set_title('Number of Overlapping Predictions at Each Time Step')
    ax1.legend(loc='upper left')
    ax1.grid(True, alpha=0.3)
    
    # ===== Plot 2: Point-wise MSE/MAE =====
    ax2 = axes[1]
    ax2.plot(time_steps, mse, 'r-', label='Avg MSE', linewidth=1, alpha=0.5)
    ax2.plot(time_steps, mae, 'b-', label='Avg MAE', linewidth=1, alpha=0.5)
    ax2.plot(time_steps, smoothed_mse, 'r-', label='Smoothed Avg MSE', linewidth=2)
    ax2.plot(time_steps, smoothed_mae, 'b-', label='Smoothed Avg MAE', linewidth=2)
    ax2.axhline(y=value_threshold, color='black', linestyle='--', linewidth=2,
    ...
    if value_warning_times:
        ax2.scatter(value_warning_times, 
                   [smoothed_mse[t] for t in value_warning_times],
                   color='red', s=30, zorder=5, alpha=0.7, label='Value Warning Points')
    ax2.set_xlabel('Time Step')
    ax2.set_ylabel('Error')
    ax2.set_title('Averaged MSE/MAE from Individual Predictions (Method 2)')
    ax2.legend(loc='upper left')
    ax2.grid(True, alpha=0.3)
    
    # ===== Plot 3: Gradient of MSE/MAE =====
    ax3 = axes[2]
    ax3.plot(time_steps, gradient_mse, 'r-', label='MSE Gradient', linewidth=1.5)
    ax3.plot(time_steps, gradient_mae, 'b-', label='MAE Gradient', linewidth=1.5)
    ax3.axhline(y=gradient_threshold, color='black', linestyle='--', linewidth=2,
    ...
    if gradient_warning_times:
        ax3.scatter(gradient_warning_times, 
    ...
    ax3.set_title('Gradient of Averaged MSE/MAE')
    ax3.legend(loc='upper left')
    ax3.grid(True, alpha=0.3)
    
    # ===== Plot 4: CUSUM (MAE 기반) =====
    ax4 = axes[3]
    # CUSUM 계산 (MAE 기준, main에서 계산된 값 사용)
    # main에서 cusum_up, cusum_down, UCL, LCL을 전달받아야 함
    if hasattr(plot_analysis, 'cusum_up') and hasattr(plot_analysis, 'cusum_down'):
        ...
        ax4.plot(time_steps, cusum_up, 'r-', label='CUSUM Up (MAE)', linewidth=1.5)
        ax4.plot(time_steps, cusum_down, 'b-', label='CUSUM Down (MAE)', linewidth=1.5)
        ax4.axhline(y=UCL, color='r', linestyle='--', label=f'UCL: {UCL:.4f}')
        ax4.axhline(y=LCL, color='b', linestyle='--', label=f'LCL: {LCL:.4f}')
        ...
        if cusum_up_warnings:
            ax4.scatter([time_steps[t] for t in cusum_up_warnings], [cusum_up[t] for t in cusum_up_warnings],
                        color='red', s=20, zorder=5, label='CUSUM_UP Warning')
        if cusum_down_warnings:
            ax4.scatter([time_steps[t] for t in cusum_down_warnings], [cusum_down[t] for t in cusum_down_warnings],
                        color='blue', s=20, zorder=5, label='CUSUM_DOWN Warning')
        ax4.set_title('CUSUM')
        ax4.legend()
        ax4.grid(True, alpha=0.3)
    else:
        ax4.text(0.5, 0.5, 'CUSUM 정보 없음', ha='center', va='center', fontsize=14)
        ax4.set_title('CUSUM')
        ax4.grid(True, alpha=0.3)
    plt.tight_layout()
    # Save figure
    ...
    save_path = os.path.join(save_dir, f'1anomaly_detection_engine{engine_idx+81}_ch{channel}_DF{DISCOUNT_FACTOR}_CUSUM.png')
