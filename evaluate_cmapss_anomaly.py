...
def compute_overlapping_predictions(trues, preds, sample_start, sample_end, channel, seq_len, pred_len, engine_len):
    """
    Compute averaged predictions and averaged errors at each time step.
    
    Method 2: Calculate MSE/MAE for each prediction individually, then average.
    This captures both bias and uncertainty in predictions.
    
    At time step t (relative to engine start):
    - Predictions from step t-pred_len, t-pred_len+1, ..., t-1 all predict time t
    - We compute error for each prediction and average them
    
    Returns:
        time_steps: array of time step indices
        actual_values: actual sensor values at each time step
        avg_predictions: averaged prediction at each time step
        avg_mse: averaged MSE from all predictions at each time step
        avg_mae: averaged MAE from all predictions at each time step
        prediction_counts: how many predictions contributed to each average
    """
    engine_trues = trues[sample_start:sample_end]  # (n_samples, pred_len, n_channels)
    engine_preds = preds[sample_start:sample_end]  # (n_samples, pred_len, n_channels)
    n_samples = engine_trues.shape[0]
    
    # Total time steps = engine_len (full engine lifecycle)
    total_time_steps = engine_len
    
    # Arrays to accumulate predictions and errors
    prediction_sum = np.zeros(total_time_steps)
    mse_sum = np.zeros(total_time_steps)  # Sum of squared errors
    mae_sum = np.zeros(total_time_steps)  # Sum of absolute errors
    prediction_count = np.zeros(total_time_steps)
    actual_values = np.full(total_time_steps, np.nan)
    
    # Fill in predictions from each sample
    # Sample i covers time steps: seq_len+i to seq_len+i+pred_len-1
    for sample_idx in range(n_samples):
        ...
        for j in range(pred_len):
            ...
            if t < total_time_steps:
                ...
                prediction_sum[t] += pred_val
                prediction_count[t] += 1
                
                # Compute individual error and accumulate
                error = pred_val - true_val
                mse_sum[t] += error ** 2
                mae_sum[t] += np.abs(error)
    ...
    avg_predictions = np.full(total_time_steps, np.nan)
    avg_mse = np.full(total_time_steps, np.nan)
    avg_mae = np.full(total_time_steps, np.nan)
    
    avg_predictions[valid_mask] = prediction_sum[valid_mask] / prediction_count[valid_mask]
    avg_mse[valid_mask] = mse_sum[valid_mask] / prediction_count[valid_mask]
    avg_mae[valid_mask] = mae_sum[valid_mask] / prediction_count[valid_mask]
    
    # Full overlap starts at time step seq_len + pred_len - 1 
    # (where pred_len predictions overlap)
...
def plot_analysis(result, mse, mae, gradient_mse, gradient_mae, smoothed_mse, smoothed_mae,
    ...
    save_path = os.path.join(save_dir, f'anomaly_detection_engine{engine_idx+81}_ch{channel}.png')
...
def main():
    ...
    result = compute_overlapping_predictions(
        trues, preds, sample_start, sample_end, CHANNEL, SEQ_LEN, PRED_LEN, engine_len
    )
