"""Genesis CUSUM Anomaly Detection Animation
...
def find_result_path(seq_len, pred_len):
    ...
    candidates = [f for f in os.listdir(results_dir)
                  if DATASET_TAG in f and f'seq{seq_len}' in f and f'pred{pred_len}' in f]
    if not candidates:
        raise FileNotFoundError(f"{DATASET_TAG} result folder not found")
...
def main():
    print("=" * 60)
    print(f"{DATASET_TAG} CUSUM Animation Generator")
    print("=" * 60)
    ch = CHANNEL_TO_SHOW
    ...
    target_mean, sigma, slack_b, ucl, lcl = compute_val_params(val_trues, val_preds, ch, SEQ_LEN, PRED_LEN, DISCOUNT_FACTOR)
    ...
    test_error, test_actual, test_pred, cusum_up_full, cusum_down_full = \
        compute_error_and_cusum_streaming(trues, preds, ch, SEQ_LEN, PRED_LEN, DISCOUNT_FACTOR, target_mean, slack_b)
    focus_start, focus_end, focus_point = find_anomaly_focus_range(
        gt_labels_test, cusum_up_full, ucl, ANOM_FOCUS_MARGIN, manual_center=FOCUS_CENTER, manual_start=FOCUS_START, manual_end=FOCUS_END)
    print(f"  Focus range: [{focus_start}, {focus_end}] (center={focus_point})")
    frame_indices = list(range(focus_start, focus_end, FRAME_STEP))
    ...
    ax_ts.plot(bg_x, test_actual[x_lo:x_hi], color='#888888', linewidth=0.6, alpha=0.5, label='Reconstructed Actual')
    ...
    line_pred_true, = ax_ts.plot([], [], color='#4CAF50', linewidth=1.2, label='Actual (pred horizon)', linestyle='--', alpha=0.8)
    vline_ts = ax_ts.axvline(x=0, color='#9C27B0', linestyle=':', linewidth=1, alpha=0.7, label='Pred start')
    ...
    if gt_labels_test is not None:
        ...
        if gt_len > x_lo:
            ...
            ax_ts.fill_between(gt_x, ymin, ymax, where=gt_sub == 1, color='orange', alpha=0.15, label='GT Anomaly', zorder=0)
            ax_ts.set_ylim(ymin, ymax)
    ax_ts.legend(loc='upper left', fontsize=7, ncol=6)
    info_text = ax_ts.text(0.98, 0.95, '', transform=ax_ts.transAxes, fontsize=9, verticalalignment='top', horizontalalignment='right',
                           bbox=dict(boxstyle='round,pad=0.4', fc='#FFFDE7', ec='#FFC107', alpha=0.9), family='monospace')
    ...
    if gt_labels_test is not None:
        ...
        if gt_len > x_lo:
            ...
            ax_cusum.fill_between(gt_x, ymin_c, ymax_c, where=gt_sub == 1, color='orange', alpha=0.1, zorder=0)
    ...
    save_dir = f'./figures_{DATASET_TAG}'
