"""GECCO CUSUM Anomaly Detection Animation
...
CHANNEL_TO_SHOW = 3         # 시각화할 센서 인덱스 (0=Tp, 1=Cl, 2=pH, 3=Redox, ...)
...
# CSV index → reconstructed time 변환: csv_idx - 14905
FOCUS_CENTER = None
FOCUS_START = None
FOCUS_END = None
# ==================================================
...
def find_result_path(seq_len, pred_len):
    ...
    if not candidates:
        raise FileNotFoundError("GECCO result folder not found")
...
def main():
    print("=" * 60)
    print("GECCO CUSUM Animation Generator")
    print("=" * 60)
    ...
    fig.suptitle(f'GECCO iTransformer — {sensor_name} Sensor | CUSUM Anomaly Detection',
    ...
    # Warning text removed as per user request
    ...
    gif_path = os.path.join(save_dir,
                            f'GECCO_CUSUM_Animation_{sensor_name}_DF{DISCOUNT_FACTOR}{focus_tag}.gif')
